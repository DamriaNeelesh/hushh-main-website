  ...
  React.useEffect(() => {
    const hydrate = async () => {
      try {
        ...
        // const prefs = JSON.parse(sessionStorage.getItem("hushh_social_preferences") || "{}");

        // Resolve email from auth, query param, or the current browser session.
        const urlEmail = typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("email") : null;
        const resolvedEmail = user?.email || urlEmail || sessionStorage.getItem("hushh_last_email") || "";
        if (!resolvedEmail) {
          setError("No user email found. Please login again.");
          setLoading(false);
          return;
        }
        try { sessionStorage.setItem("hushh_last_email", resolvedEmail); } catch {}
