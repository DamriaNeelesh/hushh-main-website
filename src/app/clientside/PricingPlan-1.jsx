...
const ClientPricingPage = () => {
  ...
  const [_activeAccordionIndex, setActiveAccordionIndex] = useState(-1);
  const [isYearly, setIsYearly] = useState(false);
  const [_isMonthly, setIsMonthly] = useState(true);
  ...
  const _handleSwitchChange = () => {
    setIsYearly(!isYearly); // toggle the state on each click
    setIsMonthly(false);
  };

  const _handlePlanCategoryChange = (category) => {
    setActivePlanCategory(category);
    setTabIndex(0);
  };

  const _renderPrice = (price, perMonth) => {
    ...
    const _durationText = isYearly ? "/yr" : perMonth;
    return `$${total}`; // Total price with updated duration text
  };

  const _yearlyTextStyles = isYearly ? { color: "white" } : { color: "#717171" };
  const _monthlyTextStyles = !isYearly ?
  { color: "white" } :
  { color: "#717171" };
                ...
                <>
                    ...
                    className="plan">
                      ...
                      <React.Fragment key={groupIndex}>
                        ...
                        <Box
                          key={`${groupIndex}-${featureIndex}-${feature}`}
                          display={"flex"}
                          flexDirection={"row"}
