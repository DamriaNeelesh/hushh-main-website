...
const PLAID_URLS = {
  production: "https://production.plaid.com/asset_report/pdf/get",
  sandbox: "https://sandbox.plaid.com/asset_report/pdf/get",
};

export async function POST(request) {
  try {
    const body = await request.json();
    const { asset_report_token, options, environment = "production" } = body;
    ...
    if (response.ok) {
      ...
      return new NextResponse(buffer, {
        ...
        headers: {
          ...
          "Plaid-Request-ID": response.headers.get("Plaid-Request-ID") || "",
        },
      });
    }
    ...
    return NextResponse.json({ error: error?.message || "Asset report PDF error" }, { status: 500 });
  }
}
