...
const PLAID_URLS = {
  production: "https://production.plaid.com/asset_report/get",
  sandbox: "https://sandbox.plaid.com/asset_report/get",
};

export async function POST(request) {
  try {
    const body = await request.json();
    const { asset_report_token, include_insights, fast_report, options, environment = "production" } = body;
    ...
    return NextResponse.json({ error: error?.message || "Asset report get error" }, { status: 500 });
  }
}
